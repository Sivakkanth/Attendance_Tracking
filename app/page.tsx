  ...
  const exportToGoogleSheet = async () => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        setError(data.error || 'Failed to export to Google Sheet');
      }
    } catch (err: any) {
      setError(err.message || 'Failed to export to Google Sheet');
    } finally {
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <button
              ...
              disabled={filteredUsers.length === 0 || sheetLoading}
              className="px-6 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {sheetLoading ? 'Exporting...' : 'Export Google Sheet'}
            </button>
          </div>
        </div>
